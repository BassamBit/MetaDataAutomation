...
# ========= Dictionary Mode (original filling with formatters) =========
...
if mode == "Dictionary":
    ...
    # TOC column selection (you can hardcode defaults; user can change to match their df)
    st.markdown("**TOC column mapping (from your DataFrame):**")
    TOC_COL1 = st.text_input("TOC Column 1 (take first line, strip trailing colon)", value="المصطلح وتعريفه  ")
    TOC_COL3 = st.text_input("TOC Column 3 (take first line, strip trailing colon)", value="Term")
    TOC_COL4 = st.text_input("TOC Column 4 (copy as-is)", value="Code")
...
if run:
    try:
        ...
        if mode == "Dictionary":
            ...
            if filled_rows < len(df):
                st.warning(f"Not all rows were written ({filled_rows} / {len(df)}). "
                           "Add more prepared slides to your template and rerun.")

        else:
            # Index (TOC)
            # Validate columns
            for col_name in [TOC_COL1, TOC_COL3, TOC_COL4]:
                if col_name not in df.columns:
                    st.error(f"Column not found in DataFrame: {col_name}")
                    st.stop()

            toc_df = df.copy()
            col1_vals = toc_df[TOC_COL1].map(first_line_no_trailing_colon)
            col2_vals = [""] * len(toc_df)  # empty on purpose (to be filled later manually)
            col3_vals = toc_df[TOC_COL3].map(first_line_no_trailing_colon)
            col4_vals = toc_df[TOC_COL4]  # as-is

            # TOC matrix expected as 4 columns (you said order: 1st col, empty, 3rd col, last col)
            toc_matrix = list(zip(col4_vals, col3_vals, col2_vals, col1_vals)) # (C1, C2 empty, C3, C4)
            ...
            if available_slides < needed_slides:
                ...
                # we still proceed with available slides
            ...
            for slide in prs.slides:
                if slide_counter >= needed_slides:
                    break  # we've filled enough slides for TOC
                tables = get_tables_sorted_by_x(slide)
                if len(tables) < 2:
                    # Template problem: TOC requires two tables per slide
                ...
                for r in range(ROWS_PER_TABLE):
                    ...
                    # write from row 0 (include header row as data if your table is headerless)
